...
{#if isTerminal}
  ...
  <div class="flex items-center gap-2">
    <span
      class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold {isCancelled
  ...
  <div class="flex items-center">
    {#each steps as step, i (step.key)}
      {#if i > 0}
        <!-- Connector -->
        <div
          class="flex-1 h-0.5 mx-1 {i <= stepIndex
            ? 'bg-emerald-400'
            : 'bg-gray-200'}"
        ></div>
      {/if}
      ...
      <div class="flex flex-col items-center gap-1.5">
        ...
        <span
          class="text-[10px] leading-tight {i <= stepIndex
