  ...
  {:else}
    <div class="divide-y divide-gray-100">
      {#each methods as method (method.id)}
        <div
          class="p-4 px-6 flex items-center justify-between hover:bg-gray-50/50 transition-colors group"
        >
          <div>
            <div class="flex items-center gap-2 mb-1">
              ...
              {#if method.isActive}
                <span
                  class="inline-flex items-center gap-1 text-[10px] font-semibold text-emerald-700 bg-emerald-50 px-1.5 py-0.5 rounded-md"
                >
                  <CheckCircle2 class="w-3 h-3" /> Active
                </span>
              {:else}
                <span
                  class="inline-flex items-center gap-1 text-[10px] font-semibold text-gray-600 bg-gray-100 px-1.5 py-0.5 rounded-md"
                >
            ...
            <p class="text-xs text-gray-500 font-mono">
              ...
              {#if method.freeShippingThreshold}
                <span
                  class="ml-2 text-[10px] bg-blue-50 text-blue-700 px-1.5 py-0.5 rounded"
                  >Free over ${(method.freeShippingThreshold / 100).toFixed(
                    2,
                  )}</span
                >
              {/if}
    ...
    >
      ...
      >
        <h3 class="text-lg font-semibold text-gray-900">
          {editId ? "Edit Shipping Method" : "Add Shipping Method"}
        </h3>
      </div>
      ...
      <div class="p-6 space-y-4">
        ...
        <div>
          <label
            for="freeShippingThreshold"
            class="block text-xs font-semibold text-gray-600 uppercase tracking-wider mb-2"
            >Free Shipping Threshold (Cents, Optional)</label
          >
          <input
