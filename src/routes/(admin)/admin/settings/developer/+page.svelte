  ...
  async function runSeed() {
    ...
    try {
      ...
      if (result.categoriesCreated)
        parts.push(`${result.categoriesCreated} categories`);
      if (result.productsCreated)
        parts.push(`${result.productsCreated} products`);
      if (result.servicesCreated)
        parts.push(`${result.servicesCreated} services`);
      if (result.eventsCreated) parts.push(`${result.eventsCreated} events`);
      seedSummary = parts.length
        ? `Created ${parts.join(", ")}.`
        : "Nothing created.";
...
<div class="bg-white border border-gray-200 rounded-xl overflow-hidden">
  ...
  <div class="p-6 space-y-5">
    ...
    <div class="grid grid-cols-3 gap-3">
      {#each [{ key: "products", label: "Catalog", icon: Package, hint: `${seedTotals.categories} categories · ${seedTotals.products} products` }, { key: "services", label: "Bookings", icon: CalendarDays, hint: `${seedTotals.services} services` }, { key: "events", label: "Events", icon: Ticket, hint: `${seedTotals.events} events` }] as opt}
        <label
          class="bg-white border border-gray-200 rounded-xl p-4 cursor-pointer hover:border-gray-300 transition-colors block"
        >
          <input
            type="checkbox"
            bind:checked={
              seedScope[opt.key as "products" | "services" | "events"]
            }
            class="sr-only"
          />
          <div class="flex items-center justify-between mb-2">
            <span
              class="flex items-center gap-1.5 text-xs font-semibold text-gray-500 uppercase tracking-wider"
            >
    ...
    {#if seedSummary}
      <div
        class="flex items-start gap-2 text-sm text-emerald-700 bg-emerald-50 border border-emerald-200 rounded-lg px-3 py-2"
      >
    ...
    {#if seedErrors.length}
      <div
        class="text-sm text-red-700 bg-red-50 border border-red-200 rounded-lg px-3 py-2 space-y-1"
      >
        <div class="flex items-center gap-2 font-medium">
          <AlertCircle class="w-4 h-4" />
          <span
            >{seedErrors.length} item{seedErrors.length === 1 ? "" : "s"} failed</span
          >
        </div>
    ...
    <div class="flex justify-end pt-2 border-t border-gray-100">
      <button
        onclick={requestSeed}
        disabled={seeding ||
          (!seedScope.products && !seedScope.services && !seedScope.events)}
