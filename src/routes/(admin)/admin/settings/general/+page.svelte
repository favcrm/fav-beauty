<script lang="ts">
  ...
  onMount(async () => {
    try {
      ...
      cutoffHours =
        bookingConfig.memberCancellationCutoffHours?.toString() ?? "";
    ...
    try {
      const [profileRes, bookingRes, loginRes] = await Promise.all([
        ...
        adminSettingsApi.updateBookingConfig({
          ...
          memberCancellationCutoffHours: cutoffHours
            ? parseInt(cutoffHours, 10)
            : null,
        }),
    ...
    <div class="p-6 space-y-6">
      <div class="flex items-start gap-6">
        <div class="flex flex-col items-center gap-3">
          <div
            class="w-20 h-20 bg-gray-100 border border-gray-200 rounded-2xl flex items-center justify-center overflow-hidden flex-shrink-0"
          >
            {#if profile?.logo_url || profile?.logoUrl}
              <img
                src={profile.logo_url || profile.logoUrl}
                alt="Logo"
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                class="w-full bg-gray-50 border border-gray-200 rounded-lg px-3.5 py-2.5 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-slate-800/20 focus:border-slate-400 transition-all"
              />
            </div>
            <div>
              <label
  ...
  <div class="bg-white border border-gray-200 rounded-xl overflow-hidden">
    ...
    <div class="p-6 space-y-6">
      ...
      <div>
        ...
        <p class="mt-1.5 text-xs text-gray-400 ml-8">
          Choose how customers receive their one-time passwords.
        </p>
      </div>
      ...
      <div class="space-y-4">
        ...
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-3">
            ...
            <div>
              <p class="text-sm font-medium text-gray-900">
                Allow Member Cancellation
              </p>
              <p class="text-xs text-gray-500">
                Let customers cancel bookings via the portal
              </p>
            </div>
          </div>
          <label class="relative inline-flex items-center cursor-pointer">
            <input
              type="checkbox"
              bind:checked={allowMemberCancellation}
              class="sr-only peer"
            />
            <div
      ...
      {#if error}
        <p
          class="text-sm text-red-600 bg-red-50 border border-red-200 rounded-lg px-4 py-2.5 mt-4"
        >
          {error}
        </p>
      {/if}
      ...
      >
        <div
          class="flex items-center gap-2 text-sm text-emerald-600 transition-all duration-300"
