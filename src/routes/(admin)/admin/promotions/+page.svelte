<script lang="ts">
  ...
  async function handleCreate() {
    ...
    try {
      ...
      if (createStartsAt)
        input.startsAt = new Date(createStartsAt).toISOString();
...
<div>
  ...
  {#if actionError}
    <div
      class="p-3 bg-red-50 text-red-700 rounded-lg text-sm mb-4 flex items-center justify-between"
    >
      <span>{actionError}</span>
      <button
        onclick={() => (actionError = "")}
        class="text-red-500 hover:text-red-700 text-xs">Dismiss</button
      >
    </div>
  {/if}
      ...
      {#snippet actions()}
        {#if !search && !statusFilter}
          <Button size="sm" onclick={openCreateModal}
            ><Plus class="w-4 h-4" /> New Promotion</Button
          >
        {/if}
      {/snippet}
    </EmptyState>
          ...
          <tbody>
            {#each data.items as promotion (promotion.id)}
              ...
              >
                ...
                <td class="px-4 py-3">
                  <span class="font-mono text-xs text-gray-700 uppercase"
                    >{promotion.code}</span
                  >
                </td>
                ...
                <td class="px-4 py-3 text-right text-gray-600 tabular-nums">
                  {promotion.usageCount}{#if promotion.usageLimitTotal !== null}<span
                      class="text-gray-400"
                    >
                      / {promotion.usageLimitTotal}</span
                    >{/if}
                </td>
                <td class="px-4 py-3">
                ...
                <td class="px-4 py-3 text-gray-500"
                  >{formatDate(promotion.createdAt)}</td
                >
                <td class="px-4 py-3">
  ...
  >
    ...
    <div class="space-y-1">
      ...
      <input
        ...
        oninput={(e) => {
          createCode = e.currentTarget.value.toUpperCase();
        }}
    ...
    <div class="space-y-2">
      ...
      <div class="grid grid-cols-2 gap-2">
        <label class="flex items-center gap-2 text-sm text-gray-700">
          <input
            type="checkbox"
            bind:checked={createBookingEnabled}
        ...
        <label class="flex items-center gap-2 text-sm text-gray-700">
          <input
            type="checkbox"
            bind:checked={createEventEnabled}
            class="rounded border-gray-300"
          />
          Event
        </label>
        <label class="flex items-center gap-2 text-sm text-gray-700">
          <input
            type="checkbox"
            bind:checked={createOnlineEnabled}
            class="rounded border-gray-300"
          />
          Online
        </label>
